...
import { Save, Trash2, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, Search, Building } from "lucide-react";
...
export const EqUnidadeDialog = ({ open, onOpenChange }: EqUnidadeDialogProps) => {
  ...
  const [totalRecords] = useState(45); // Exemplo de quantidade
  const [id, setId] = useState("1");
  const [unidade, setUnidade] = useState("Batalhão de Choque");
  const [sigla, setSigla] = useState("BPChoque");
  ...
  const handleDelete = () => {
    toast.error("A unidade foi removida do cadastro.");
  };

  const handleSearch = () => {
    if (!searchTerm) return;
    toast.info(`Buscando unidade: ${searchTerm}...`);
  };
      ...
      <DialogContent className="sm:max-w-[650px] p-0 overflow-hidden border-none shadow-2xl">
          ...
          <div className="grid grid-cols-6 gap-4 p-5 rounded-2xl border border-border/60 bg-muted/20">
            ...
            <div className="col-span-5 grid grid-cols-4 gap-4">
              <div className="col-span-3 space-y-2">
                <Label htmlFor="unidade" className="text-[11px] font-black uppercase text-pmpa-navy opacity-70">
                  Nome da Unidade
                </Label>
                <Input
                  id="unidade"
        ...
        <div className="bg-muted/30 px-6 py-3 border-t border-border/50 flex justify-between items-center">
          ...
          <div className="flex gap-1">
            ...
            <span className="text-[10px] font-bold text-emerald-600 uppercase">Banco Ativo</span>
          </div>
        </div>
      </DialogContent>
    </Dialog>
