...
export const OnboardingTour = () => {
  ...
  return (
    <>
      {/* Botão flutuante para iniciar o manual - Esconde durante o tour */}
      {!active && (
        <Button
          onClick={startTour}
        ...
        {active && (
          <div className="fixed inset-0 z-[9999] pointer-events-none overflow-hidden">
            ...
            <motion.div 
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="absolute inset-0 bg-slate-950/85 pointer-events-auto"
              style={{
                clipPath: `polygon(
                  0% 0%, 
                  0% 100%, 
            ...
            <motion.div
              ...
              style={{ 
                ...
                top: steps[currentStep].position === 'bottom' ? (coords.top + coords.height + 20) : 
                     steps[currentStep].position === 'top' ? (coords.top - 220) :
                ...
                left: steps[currentStep].position === 'right' ? (coords.left + coords.width + 20) :
                      ...
                      steps[currentStep].position === 'center' ? '50%' : Math.max(20, Math.min(window.innerWidth - 320, coords.left)),
                transform: steps[currentStep].position === 'center' ? 'translate(-50%, -50%)' : 'none',
              }}
              className="z-[10000] w-[300px] bg-white dark:bg-slate-900 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] p-6 pointer-events-auto border border-blue-100 dark:border-slate-800"
            >
